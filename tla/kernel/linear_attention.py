...
def _fused_chunk_fwd(B, H, S, D, scale, dtype, BK, BV, chunk_size):
    accum_dtype = "float"
    NK = D // BK
    NV = D // BV
    NT = S // chunk_size
    ...
    def main(
        ...
        with T.Kernel(NV, NK, B * H) as (i_v, i_k, i_bh):
            ...
            T.annotate_layout({
                q:
                tl.layout.make_swizzled_layout(q),
                k:
                tl.layout.make_swizzled_layout(k),
                v:
                tl.layout.make_swizzled_layout(v),
                h_shared:
                tl.layout.make_swizzled_layout(h_shared),
                s_shared:
                tl.layout.make_swizzled_layout(s_shared),
            })
            T.use_swizzle(8)
            ...
            # Output final state
            T.copy(
                h, final_state[i_b, i_h, i_k * BK:(i_k + 1) * BK,
                               i_v * BV:(i_v + 1) * BV])

    return main


def _fused_chunk_bwd(B, H, S, D, scale, dtype, BK, BV, chunk_size):
    accum_dtype = "float"
    NK = D // BK
    NV = D // BV
    NT = S // chunk_size
    ...
    def main(
        ...
        with T.Kernel(NV, NK, B * H) as (i_v, i_k, i_bh):
            ...
            T.annotate_layout({
                ds_shared:
                tl.layout.make_swizzled_layout(ds_shared),
                q:
                tl.layout.make_swizzled_layout(q),
                k:
                tl.layout.make_swizzled_layout(k),
                ...
                tl.layout.make_swizzled_layout(v),
                do:
                tl.layout.make_swizzled_layout(do),
                h_shared:
                tl.layout.make_swizzled_layout(h_shared),
                dh_shared:
                tl.layout.make_swizzled_layout(dh_shared)
            })
...
class FusedChunk_Kernel(nn.Module):
    ...
    def __init__(self,
        ...
        assert dtype in [torch.float16, torch.bfloat16,
                         torch.float], 'dtype must be float16 or bfloat16'
        dtype = dtype.__str__().split('.')[-1]  # Convert torch dtype to string

        # Prepare fwd kernel
        self.fwd_program = _fused_chunk_fwd(batch, heads, seqlen, dim, scale,
                                            dtype, block_K, block_V,
                                            chunk_size)
        ...
        # Prepare bwd kernel
        self.bwd_program = _fused_chunk_bwd(batch, heads, seqlen, dim, scale,
                                            dtype, block_K, block_V,
                                            chunk_size)
